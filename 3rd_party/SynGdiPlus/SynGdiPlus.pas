...
{$A+}
  ...
  // Recognized types for image loading
  TImageFormat = (imgGIF, imgPNG, imgJPG, imgBMP, imgTIF, imgWMF, imgEMF, imgUndefined );   // DPV
...
{$endif}
    ...
    /// guess the picture type from its internal format, including WMF or EMF
    function GetNativeImageFormat: TImageFormat;                                       // DPV
...
{ Dado que TSynPicture realmente puede recuperar imágenes en otros formatos,

Image.GetRawFormat method. En gdiplusimaging.h:
...
// https://learn.microsoft.com/es-es/dotnet/desktop/winforms/advanced/using-image-encoders-and-decoders-in-managed-gdi?view=netframeworkdesktop-4.8
  GDI+ tiene codificadores y descodificadores integrados que admiten los siguientes tipos de archivo:     BMP, GIF, JPEG, PNG, TIFF
  GDI+ también tiene descodificadores integrados que admiten los siguientes tipos de archivo:             WMF, EMF, ICON

 -> GDI+ puede leer contenidos WMF o EMF, pero no convertir imágenes a ese formato
}

function TSynPicture.GetNativeImageFormat: TImageFormat;
const // only the TGUID.D1 is relevant here
  RawFormat: array[TImageFormat] of cardinal =
    ($b96b3cb0, $b96b3caf, $b96b3cae, $b96b3cab, $b96b3cb1, $b96b3cad, $b96b3cac, 0);                   // imgGIF, imgPNG, imgJPG, imgBMP, imgTIF, imgWMF, imgEMF, imgUndefined
var id: TGUID;
begin
  if Gdip.Exists and fHasContent and (fImage<>0) and
    ...
    for result := low(result) to high(result) do
      if id.D1=RawFormat[result] then
        exit;
  result := imgBMP; // by default, returns bitmap
end;
