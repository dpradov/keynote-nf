...
begin
  ...
  begin
    ...
    begin
      FPrevWndProc := P;
      SetWindowLong(FControl.Handle, GWL_WNDPROC, {$IFDEF RX_D16}LONG_PTR{$ELSE}Longint{$ENDIF}(FNewWndProc));
    end;
  end;
end;
...
begin
  if Assigned(FControl) then begin
    ...
    (Pointer(GetWindowLong(FControl.Handle, GWL_WNDPROC)) = FNewWndProc) then
      SetWindowLong(FControl.Handle, GWL_WNDPROC, {$IFDEF RX_D16}LONG_PTR{$ELSE}Longint{$ENDIF}(FPrevWndProc));
  end;
  FPrevWndProc := nil;
end;
    ...
    begin
      ...
      begin
        ...
        if Assigned(HookList) and not (FDestroying or
          ...
          PostMessage(HookList.FHandle, CM_RECREATEWINDOW, 0, {$IFDEF WIN64}NativeInt{$ELSE}LongInt{$ENDIF}(Self));
      end;
    end;
  end;
end;
...
{$ENDIF}
    if Assigned(Hook) and (Hook.FList.Count = 0) and Assigned(HookList) then
      PostMessage(HookList.Handle, CM_DESTROYHOOK, 0, {$IFDEF WIN64}LPARAM{$ELSE}LongInt{$ENDIF}(Hook));
