...
begin
  ColorCount := 0;
  Stream.Position := 0;
  BI := PBitmapInfoHeader({$IFDEF WIN64}IntPtr{$ELSE}LongInt{$ENDIF}(Stream.Memory) + SizeOf(TBitmapFileHeader));
  W := BI^.biWidth; H := BI^.biHeight;
  Pal := PRGBPalette({$IFDEF WIN64}IntPtr{$ELSE}LongInt{$ENDIF}(BI) + SizeOf(TBitmapInfoHeader));
  Bits := Pointer({$IFDEF WIN64}UIntPtr{$ELSE}LongWord{$ENDIF}(Stream.Memory) + PBitmapFileHeader(Stream.Memory)^.bfOffBits);
