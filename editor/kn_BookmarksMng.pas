...
begin
  ...
  App.ShowInfoInStatusBar(Format( GetRS(sBmk01), [Number] ));
end;
...
begin
  ...
  if assigned(aLocation) then begin
     if JumpToLocation(aLocation) then
        App.ShowInfoInStatusBar(Format( GetRS(sBmk04), [Number] ))
     else begin
        BookmarkClear (Number, false);
        App.ShowInfoInStatusBar(Format( GetRS(sBmk03), [Number] ));
     end;
  end
  else
      App.ShowInfoInStatusBar(Format( GetRS(sBmk02), [Number] ));
end;
